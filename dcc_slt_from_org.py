...
if __name__ == '__main__':
    ...
    if len(args) == 2:  # 需要跟俩个参数执行
        ...
        # 卫星标识检查，配置中没有则不处理
        if satFlag not in inCfg['TYPE']['satType']:
...
#             print LatRange[0], LatRange[1]
            ...
            # 6.2 计算des
            des = dcc_cal_des(ymd)
            shape_x = dcc.DN_Avg_Std.shape[0]
            shape_y = len(idx[0])
            shape_z = 5
#             print shape_x
            DN_ADMs = np.full((shape_x, shape_y, shape_z), 0)
            Ref_ADMs = np.full((shape_x, shape_y, shape_z), 0)
#             print DN_ADMs.shape
            col_id_Lst = [0, 1, 3, 5, 7]
            for i in col_id_Lst:

                j = col_id_Lst.index(i)
                dnValue = dcc.DN_Avg_Std[:, idx[0], i]
                RefValue = dcc.Ref_Avg_Std[:, idx[0], i]
                dnValue = np.ma.masked_where(dnValue >= 65535, dnValue)
                RefValue = np.ma.masked_where(RefValue >= 65535, RefValue)
#                 DN_ADMs[:, :, j] = dcc.DN_Avg_Std[:, idx[0], i] / np.cos(np.deg2rad(SZA)) * des / ADMs_pixle1
#                 Ref_ADMs[:, :, j] = dcc.Ref_Avg_Std[:, idx[0], i] / np.cos(np.deg2rad(SZA)) * des / ADMs_pixle1

                DN_ADMs[:, :, j] = dnValue / np.cos(np.deg2rad(SZA)) * des / ADMs_pixle1
                Ref_ADMs[:, :, j] = RefValue / np.cos(np.deg2rad(SZA)) * des / ADMs_pixle1

#                 print '1' , i, dcc.DN_Avg_Std[:, idx[0], i]
#                 print '2' , i, dcc.Ref_Avg_Std[:, idx[0], i]
            print DN_ADMs.shape, Ref_ADMs.shape
            ...
            DccDataWrite(dcc, idx[0], DN_ADMs, Ref_ADMs, ADMs_pixle1, outFile)
            date_s = date_s + relativedelta(days=1)
