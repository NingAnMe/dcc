...
#             sys.exit(-1)
        ...
        for each in var:
            for ch in band:
                FileName = 'DCC_%s_%s_%s_Rolldays_%s_ALL.txt' % (satFlag, each, ch, rollday)
                ...
                ######## 三、绘图   ###################
                outPng_avg = os.path.join(opath, 'DCC_%s_%s_%s_Rolldays_%s_Timeseries_Avg.png' % (satFlag, each, ch, rollday))
                outPng_med = os.path.join(opath, 'DCC_%s_%s_%s_Rolldays_%s_Timeseries_Med.png' % (satFlag, each, ch, rollday))
                outPng_mod = os.path.join(opath, 'DCC_%s_%s_%s_Rolldays_%s_Timeseries_Mod.png' % (satFlag, each, ch, rollday))
                x = []
...
#                 p.y_fmt = "%0.3f"
                p.easyplot(x, AvgATT, 'r', 'avg', marker='x-', markersize=3, mec="r")
                ...
                p.fig.subplots_adjust(bottom=0.15)
#                 p.y_fmt = "%0.3f"
                p.easyplot(x, MedATT, 'r', 'med', marker='x-', markersize=3, mec="r")
                p.title = u'%s %s 时序图' % (satFlag, ch)
                ...
                p.fig.subplots_adjust(bottom=0.15)
#                 p.y_fmt = "%0.3f"
                p.easyplot(x, ModATT, 'r', 'mod', marker='x-', markersize=3, mec="r")
                p.title = u'%s %s 时序图' % (satFlag, ch)
